...
import { ArrowLeft, Plus, Trash2, Download, Search, X, AlertCircle, MoreVertical } from 'lucide-react';
import {
  ...
  return (
    <Modal isOpen title="Edit Subscriber" onClose={onClose} size="md">
      <div className="space-y-4">
        ...
        <div>
          <label htmlFor="list-detail-subscriber-name" className="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">Name</label>
          <input
            id="list-detail-subscriber-name" name="subscriber_name"
            type="text"
            value={name}
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="flex items-start gap-3 mb-1">
        <button
          onClick={onBack}
          className="mt-1 p-1.5 rounded-lg text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex-shrink-0"
        >
          <ArrowLeft size={18} />
        </button>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white truncate">{list?.name}</h2>
      ...
      <div className="flex items-center justify-between mb-6 pl-10">
        <p className="text-sm text-gray-400">{subscribers.length} subscriber{subscribers.length !== 1 ? 's' : ''}</p>
        {selectedIds.length > 0 && (
